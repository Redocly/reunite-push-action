...
// The published @redocly/cli package is a self-contained bundle (since
// v2.34.0) that no longer exposes its commands as importable modules, so the
// action invokes the CLI as a child process instead. `npm run package`
// vendors the CLI next to the action bundle.
export function getRedoclyCliPath(): string {
  const vendoredCliPath = path.join(
  ...
  if (fs.existsSync(vendoredCliPath)) {
    return vendoredCliPath;
  }

  // Fallback for running the action from sources.
  return path.join(
    process.cwd(),
    'node_modules',
    '@redocly',
...
async function runRedoclyCli(args: string[]): Promise<string> {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [getRedoclyCliPath(), ...args], {
      env: process.env,
    });
